...
export interface HermesDeliverySubmission {
  /** Stable Schedule delivery identity. A client must use it for idempotency and reconciliation. */
  readonly dedupeKey: string;
  /** Bounded display text; no Schedule credential, claim token, or separate intent ID is exposed. */
...
function redactedResult<Result>(operation: () => Result): Result {
  try {
    return operation();
  } catch {
    return ambiguous();
...
function reconciliationOutcome(value: unknown): "accepted" | "not_found" {
  return redactedResult(() => {
    const result = exactDataObject(value, ["outcome"]);
    if (result === null || typeof result.outcome !== "string") return ambiguous();
    if (result.outcome === "accepted" || result.outcome === "not_found") return result.outcome;
    return ambiguous();
  });
}
...
function submissionOutcome(value: unknown): ReminderTransportResult {
  return redactedResult(() => {
    const discriminator = exactDataObject(value, ["outcome"]);
    if (discriminator?.outcome === "accepted") return { outcome: "delivered" };
    if (discriminator?.outcome === "permanent_failure") {
      return { outcome: "permanent_failure", failureCode: "hermes.permanent_failure" };
    }

    const retryable = exactDataObject(value, ["outcome", "retryAfterSeconds"]);
    if (
      retryable?.outcome !== "retryable_failure" ||
      !Number.isSafeInteger(retryable.retryAfterSeconds) ||
      (retryable.retryAfterSeconds as number) < 0 ||
      (retryable.retryAfterSeconds as number) > MAXIMUM_RETRY_AFTER_SECONDS
    ) {
      return ambiguous();
    }
    return {
      ...
      retryAfterSeconds: retryable.retryAfterSeconds as number,
    };
  });
}
