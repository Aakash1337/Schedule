...
Schedule can use an optional local Ollama/Gemma model to turn free-form text into one reviewable root
backlog item, one unlinked calendar block, or one routine. For work items, the model may also return
separate review-only suggestions for priority, due date, and planning duration. For routines, the
title is the only model-authored field. Every other routine value starts as a visible native default
and remains user-reviewed. This is a deliberately narrow capture path, not a general command agent.
The model cannot create anything, call tools, read Schedule data, select a workspace, or change
planning state. A result exists only after the user reviews every value and explicitly confirms it.

## Implemented scope

The version 4 provider contract supports exactly three commands:
...
```json
{ "type": "routine.create", "title": "Practice piano" }
```
...
For `routine.create`, Schedule expands the title-only provider command into a visible, native routine
review. The initial values are active status; medium priority and effort; normal energy; neutral
preference; empty context, category, and free-form tags; a 15/30/60-minute nonsplittable duration
with no minimum session and zero overhead; and a weekly target of three with one-day minimum spacing,
consecutive days discouraged, Monday as the week start, and no minimum, maximum, weekday, date,
pause, or end constraints. The user may change the title, description, status, tags, duration, and
cadence before confirmation. The complete validated snapshot is persisted and confirmed as exactly
one routine whose ID is derived from the proposal. There are no model-authored routine settings or
routine suggestions.

Task breakdown, multiple commands, linked calendar blocks, hosted providers, plan mutations, and
Hermes/WhatsApp interpretation remain outside the contract.
...
exact versioned JSON schema and then pass independent provider and application validation of keys,
lengths, NFC normalization, whitespace, control and bidirectional-formatting characters, command
kind, and work-item, schedule-block, or routine domain rules. A block must use the trusted context
zone, end after it starts, and contain no model-controlled identifier. A provider routine command may
contain only its type and title; a full routine snapshot can enter storage only through Schedule's
native defaults or the explicit review update. Suggestions have exact keys and bounded values; an
empty or malformed suggestion object cannot cross the provider boundary.
...
eventual typed result identity, immutable advisory suggestions plus their own canonical SHA-256
digest, and the separately stored user-authored work-item review fields plus their own digest. A
routine's complete review snapshot is bound by the command digest, and its canonical display remains
bounded to 64,000 characters. Suggestions are excluded from the command and review digests,
confirmation input, and audit payloads. API responses use
...
4. The browser displays the exact proposed command. Work-item suggestions remain separate and use
   one explicit-use control per suggested field. Suggestions never prefill the user's choices. An
   edit replaces the complete reviewed snapshot under one
   ...
   type. Routine review exposes the model-authored title alongside every visible native default and
   uses the ordinary routine controls for the full user-reviewed snapshot.
...
6. Confirmation requires an `Idempotency-Key` and the expected proposal version. One serializable
   ...
   command digest and applicable reviewed-field digest, creates the deterministic root backlog work
   item, unlinked calendar block, or routine, marks the proposal confirmed, and appends the audit
   event.

New generation requires `schedule.natural-language/v4`. Version 3 is accepted only to recover an
already-pending proposal whose workspace, request ID, normalized prompt, reference date, and time
zone match its version-3 fingerprint. That compatibility request never calls the provider or inserts
a proposal, and its successful response is always a version-4 envelope. A version-3 request without
that exact pending winner is rejected and cannot start generation.
...
controls as Calendar and remains explicitly unlinked. A routine uses the native status, tag,
duration, and cadence controls; none is inferred by the model. The matching **Create** action first
persists a changed full snapshot when needed and then confirms with one stable key; an ambiguous
network retry reuses that key. After success, the panel closes; a returned backlog item is merged
into the board and focused, a calendar result is available on the Calendar view, and a routine result
is available in Routines. Workspace changes abort and discard in-flight work.
...
review fields, migration `0038` adds nullable advisory suggestions, migration `0039` adds the
tenant-bound calendar-block result plus command-typed lifecycle constraint, and migration `0040`
adds the tenant-bound routine result to that typed lifecycle and widens the canonical command-display
constraint from 1,000 to 64,000 characters for the full routine snapshot. The table is included in
the exact backup and restore catalog. With PostgreSQL running,
`pnpm verify:natural-language-proposals` checks private persistence, immutable suggestion isolation,
no mutation during routine review, an exact reviewed routine whose canonical display exceeds 1,000
characters, exact reviewed work-item, calendar-block, and routine results, tenant isolation,
same-key replay, competing-key conflict, and exactly one result and confirmation audit under real
concurrent transactions. `pnpm verify:web-e2e` uses the production
...
- A routine title is the only model-authored routine field. Tags, duration, cadence, description,
  and status begin as visible native defaults and remain user-reviewed.
...
- Task breakdown, multi-command capture, model-authored routine settings, linked blocks, hosted
  providers, and Hermes interpretation remain deferred.
