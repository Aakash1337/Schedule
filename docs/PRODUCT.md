...
- Implemented foundation: deterministic reminder profiles, rules, one-offs, exact-once intent
  ...
  single-flight polling with loopback health and graceful shutdown. An inert Hermes/WhatsApp bridge
  adds reconcile-before-send semantics, bounded outcome mapping, and sanitized minimal message
  projection, while a live provider client, provider-specific reconciliation, external
  bootstrap/control wiring, and human/account binding remain follow-on work
