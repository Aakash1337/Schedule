...
- Work-item due dates plus a deterministic reminder-policy and intent-materialization core; automatic execution and delivery remain deferred
...
This is not a delivery claim. Materialization is currently an explicit local API command. No worker
runs it periodically; no intent is claimed or revalidated for delivery; no message, recipient,
provider, channel, conversation, acknowledgement, or receipt is stored. Those responsibilities stay
...
- Implemented foundation: deterministic reminder profiles, rules, one-offs, and exact-once intent
  materialization; periodic execution, delivery resolution, receipts, and settings UI are separate
  follow-on work
