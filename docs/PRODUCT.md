...
Implementation note: deterministic Phase 1 is implemented across the domain, application use cases, PostgreSQL adapters, schema, migrations, unit tests, and seeded simulation coverage. Phase 2 now includes stable typed plan-item identities, an authoritative Today-plan head, audited lock and activity state, immutable regeneration/replacement and alternative-selection revisions, deterministic read-only comparison of up to three distinct alternatives, routine-only **Not today** and **Not this week** feedback, status-based backlog/Kanban work items with direct prerequisites and arbitrary-depth subtasks, bounded non-recurring calendar-block management, opt-in calendar-aware first-plan availability, and a responsive local web interface. Planner v8 selects both reusable routines and explicitly opted-in one-time leaf work items, applies temporary routine feedback, explicit bounded routine selection preferences, fixed versioned post-target repetition decay, and recent skipped/deferred/dismissed fatigue, and hard-excludes parent containers and work with unmet prerequisites from new selection and unlocked regeneration retention. A locked nonterminal item remains anchored under the existing user-authority rules. The planner also adds transparent deadline pressure for eligible work. Phase 3 now includes a transparent routine-duration insight with explicit approval, evidence-backed Daily Plan Fit guidance with explicit use receipts and read-only outcome history, and reversible user-authored routine ranking preferences; none applies automatically, and the descriptive history does not enter planner scoring. Broader inferred preferences and automatic adaptation remain deferred. Phase 4 now has an opt-in, read-only local advisor behind a provider-neutral application port: the Today interface can ask an allowlisted local Gemma model through Ollama for bounded structured suggestions, but neither the provider nor its output can mutate or replace the deterministic plan. The Work interface may separately prepare one expiring, editable root-backlog item, unlinked calendar block, or routine proposal. A routine title is the only model-authored routine field; every other setting starts from visible native defaults and remains user-reviewed. No result exists until explicit, audited, exactly-once confirmation. A provider-neutral authenticated gateway provides Today reads, credential-scoped work-item discovery including hierarchy, reviewed create/reparent/detach mutations, and least-privilege reminder delivery claims/receipts for future agents. The secure outbound substrate supports operator-queued tests and an explicit opt-in, privacy-thin `schedule.changed.v1` invalidation without schedule content. A deterministic reminder-policy core stores versioned profiles and rules, explicit one-offs, concurrency-safe immutable intents, an opt-in local periodic materializer, and a provider-neutral fenced delivery lifecycle without performing provider transport. See [API.md](./API.md), [NATURAL_LANGUAGE.md](./NATURAL_LANGUAGE.md), [REMINDERS.md](./REMINDERS.md), [WEB.md](./WEB.md), [INTEGRATIONS.md](./INTEGRATIONS.md), and [WEBHOOKS.md](./WEBHOOKS.md). Model-driven task breakdown, multi-command capture, model-authored routine settings, linked blocks, automatic advisor application or calibration, hosted model providers, generalized undo, calendar-block recurrence authoring, phone delivery, a live authenticated Hermes client and provider binding, and public hosting remain deferred.
...
Reaching a target sharply reduces selection probability. The default versioned rule divides seeded selection weight by 10 once for each completion at or beyond the target and stops at a nonzero floor of 1. Reaching a hard maximum makes the routine ineligible until the period resets. A target is not a maximum unless explicitly configured as one.
...
score =
  ...
  - category saturation
  - skip, deferral, or dismissal fatigue
...
- A versioned multiplicative repetition decay and low nonzero floor for eligible,
  target-satisfied routines
...
User-editable temperature and learned probability tuning remain deferred.
...
- Three completions satisfy the target and apply one decay step; each later completion in the same
  cadence period applies another.
- Its nonzero floor preserves a very small seeded selection probability until its maximum is reached.
...
This is based on completions, not merely suggestions. Deferrals, dismissals, and skips contribute to
the separate fatigue component.
...
- Converting natural-language requests into multiple tasks or task breakdowns; the separate proposal
  path supports only one fully reviewed routine
...
repositories, tools, or mutation services. Its valid commands are one `work_item.create` title, one
unlinked `schedule_block.create` with canonical instants and the trusted zone, or one
`routine.create` title. Model summary and warnings are transient review text.
...
reviewed-field digest, and never prefill or execute. They are forbidden for calendar-block and routine
...
For a routine, the title is the only model-authored field. Schedule supplies a full visible native
default: active status; medium priority and effort; normal energy; neutral preference; empty tags; a
15/30/60-minute nonsplittable duration; and a weekly target of three with one-day spacing,
consecutive days discouraged, and Monday as the week start. The user reviews or replaces the title,
description, status, tags, duration, and cadence as one versioned snapshot. No model-authored routine
setting or suggestion can enter that snapshot.
...
proposal and digest, creates a deterministic backlog work-item, calendar-block, or routine identity,
marks the proposal, and audits it. Same-key retries replay; a competing key conflicts. The user may
select priority, a due date, and a planning duration before work-item confirmation. A calendar review
exposes title, date, start, and end while keeping the block unlinked. A routine review exposes every
native routine field and creates nothing until confirmation. No proposal mutates the current Today
plan.

This implements natural-language creation for one reviewed root backlog item, one reviewed unlinked
calendar block, or one fully reviewed routine, including review-only suggestions for existing
work-item scheduling fields. Task breakdown, multi-command capture, model-authored routine settings,
linked blocks, automatic confirmation, prompt history, model-driven planning, hosted providers, and
Hermes/WhatsApp interpretation remain deferred. The complete privacy, lifecycle, API, and
verification contract is in [NATURAL_LANGUAGE.md](./NATURAL_LANGUAGE.md).
...
- Implemented: fixed, versioned post-target repetition decay in the seeded selection weight, with a
  nonzero exploration floor and no learned or outcome-driven tuning
- Deferred: inferred cadence, day/time, energy, preference, and category-balance signals
- Deferred: automatic application, learned or outcome-tuned probability policies, causal outcome
  analysis, upward Plan Fit expansion, editable Plan Fit policy, and algorithm comparison tools
...
- Implemented: separate free-form Work capture for one expiring, editable root backlog-item,
  unlinked calendar-block, or fully reviewed routine proposal, with prompt-private persistence and
  explicit audited exactly-once confirmation
...
- Implemented: routine title-only provider authority, visible native defaults, and a complete
  user-reviewed status/tag/duration/cadence snapshot
- Deferred: model-authored routine settings, linked blocks, multi-command capture, and task breakdown
...
- Whether production evidence warrants user-editable exploration controls beyond the fixed default
