...
- CORS remains disabled. The UI does not make the unauthenticated API remotely accessible.
...
2. **Work** groups one-time work items into the six supported status columns. Status changes use
   ...
   descriptions, priority, and status remain editable from the board.
...
exhaustion, initial loading uses announced skeleton states, and mutations keep existing data visible
while disabling only the submitted control.
