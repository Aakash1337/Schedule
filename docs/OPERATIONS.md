...
but its shared PostgreSQL dedupe store, inert reconcile-before-send client bridge, and fail-safe
supervised runtime do not make it a live provider: an authenticated Hermes client,
provider-specific conclusive reconciliation, provider/account binding, external bootstrap and
control wiring, and phone verification remain deferred. A successful webhook test or invalidation
delivery also does not verify the separate local Hermes plugin or imply that a reminder reached a
phone.
